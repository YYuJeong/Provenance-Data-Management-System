...
with open("randomData.csv",'r') as f:
    ...
    def create_activity(tx, activityType, date):
        tx.run("CREATE (activity:Activity {name: $activityType, date: $date})", activityType=activityType, date=date)
    
  
    #create entity
    def create_entity(tx, dataName, dataType, price, device):
        tx.run("CREATE (entity:Entity {name: $dataName, type: $dataType, price: $price, device: $device})", dataName=dataName, dataType=dataType, price=price, device=device)
